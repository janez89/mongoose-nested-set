...
var NestedSetPlugin = function(schema, options) {
  options = options || { root: '5182cc99f34981c90d000003' };
...
//          console.log('WARNING: tree is not built for ' + modelName + ' nodes. Parent does not have lft/rgt');
  ...
  // Returns the list of children
  schema.static('tree', function(callback) {
    var self = this;
    // find root element
    self.findOne({_id: options.root}, function(err, root){
      if (err || !root)
        return callback(err);
      self.find( { lft: { $gt : root.lft }, rgt: { $lt : root.rgt } }, function(err, docs){
        // toJSON for prevent stack execeed
        // build from element
        build_tree(root, function(data){
          callback(null, root.toJSON());
        }, 0);


        // tree stack builder
        function build_tree(el, cb, lvl){
          var i = 0;
          el.childs = [];
          el.lvl = lvl;
          async.forEachSeries(docs, function(item, next){
            i++;
            if (item.parentId.toString() == el._id.toString()){
